...
<head>
    ...
    <!-- Favicon icon -->
    <title>PGN SHARE | Forgot Password</title>
...
<section id="wrapper">
    <div class="login-register" style="background-image:url({{ asset('template/assets/images/background/bg.jpg') }});">
        <div class="login-box card model-huruf-family">
            <div class="card-body">
                <center>
                    <img class="m-t-20" src="{{ asset('template/assets/images/logo-pertamina-gas-negara.png') }}"
                         width="200px" alt="Logo PGN">
                    <h5 class="m-t-20 model-huruf-family"><b>FORGOT PASSWORD</b></h5>
                </center>
                <br>
                <form class="form-horizontal form-material" method="post" action="{{ action('LoginController@checkEmail') }}">
                    {{ csrf_field() }}
                    <div class="form-group">
                        <div class="col-xs-12">
                            <input class="form-control" type="text" name="email" placeholder="Email Address" value="{{ old('email') }}" autofocus>
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-md-12">
                            <div class="custom-control custom-checkbox">
                                <a href="{{ route('login') }}" class="text-dark pull-right"><i class="fa fa-arrow-circle-left mr-2"></i>Back to Login</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group text-center">
                        <div class="col-xs-12">
                            <button class="btn btn-block btn-lg btn-info" type="submit">CHECK EMAIL</button>
                        </div>
                    </div>
...
<script>
    @if (count($errors) > 0)
    toastr.warning('Email address harus diisi', 'Warning', {closeButton: true});
    @endif
</script>
